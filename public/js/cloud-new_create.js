...
$(document).ready(function() {
	...
	var cloudDropzone = new Dropzone(document.body, {
		...
		acceptedFiles: ".txt, .doc, .rtf, .log, .tex, .msg, .text, .wpd, .wps, .docx, .page, .csv, .dat, .tar, .xml, .vcf, .pps, .key, .ppt, .pptx, .sdf, .gbr, .ged, .mp3, .m4a, .waw, .wma, .mpa, .iff, .aif, .ra, .mid, .m3v, .e_3gp, .shf, .avi, .asx, .mp4, .e_3g2, .mpg, .asf, .vob, .wmv, .mov, .srt, .m4v, .flv, .rm, .png, .psd, .psp, .jpg, .jpeg, .tif, .tiff, .gif, .bmp, .tga, .thm, .yuv, .dds, .ai, .eps, .ps, .svg, .pdf, .pct, .indd, .xlr, .xls, .xlsx, .db, .dbf, .mdb, .pdb, .sql, .aacd, .app, .exe, .com, .bat, .apk, .jar, .hsf, .pif, .vb, .cgi, .css, .js, .php, .xhtml, .htm, .html, .asp, .cer, .jsp, .cfm, .aspx, .rss, .csr, .less, .otf, .ttf, .font, .fnt, .eot, .woff, .zip, .zipx, .rar, .targ, .sitx, .deb, .e_7z, .pkg, .rpm, .cbr, .gz, .dmg, .cue, .bin, .iso, .hdf, .vcd, .bak, .tmp, .ics, .msi, .cfg, .ini, .prf",
	...
	cloudDropzone.on("success", function(file, response) {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$(file.previewElement).attr('data-download',window.location.protocol + '//' + window.location.hostname + /*'/webroot' +*/ response.Media.url_download);
                ...
                time__ = Math.round(new Date(response.Cloud.created).getTime()/1000);
				$(file.previewElement).attr('data-created',time__);
				initDrag();
                $('div[data-id='+response.Cloud.id+'] .close__').attr('data-id',response.Cloud.id).attr('data-table', 'cloud');
	...
	function setContextMenuPosition(event) {
		...
		if ( ($('.wrapper-container').outerHeight() - menuPositionY) < menuHeight ) {
	...
	$('body').on('click', '.context-menu_link', function(event) {
		event.preventDefault();
	...
	function cloudRename(data) {
		...
		$('body').on('click', '.rename-folder_btn', function(event) {
			...
			.success(function(response){
				if(type == 'file') {
					el.find('.cloud-list_item-title .title').html('<span class="filetype">' + ext + '</span>' + newTitle);
				} else if (type == 'folder') {
			...
			if(data[key].type == 'file') {
				...
				.success(function(response) {
					...
					if(response.newTitle){
						cloneItem.find('.title').html('<span class="filetype">' + data[key].ext + '</span>' + response.newTitle);
					}
