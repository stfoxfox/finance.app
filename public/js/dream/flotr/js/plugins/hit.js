...
Flotr.addPlugin('hit', {
  ...
  hit : function (mouse) {
    ...
    if (!prevHit || (prevHit.index !== n.index || prevHit.seriesIndex !== n.seriesIndex)) {
      ...
      if (n.index != 0) {
        if (n.series && n.mouse && n.mouse.track) {
          ...
          //$(".flotr-mouse-value").css("display","block");
        }

        /*var p_html = getParsedDataForGraph(global_graph_display[n.index]['last_event']);
        if (p_html != undefined) {
          //$(".flotr-mouse-value>strong,.go_to_event").hide();
          $(".flotr-mouse-value").html(p_html);
        }
        sel_x_my = n.index;*/
    ...
    {
      if (n.index > 0 && global_graph_display.length > 0)
      {
        var elem = "";
        var index = 0;
        for (var k in eventsGraph)
        {
          if (index == n.index-1)
          {
            elem = eventsGraph[k];
        ...
        var p_html = getParsedDataForGraph(elem);
         if (p_html != undefined) {
           //$(".flotr-mouse-value>strong,.go_to_event").hide();
           $(".flotr-mouse-value").html(p_html);
         }
