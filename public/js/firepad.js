...
firepad.Firepad = (function(global) {
  ...
  var performClick = function() {
   ...
   var input = utils.elt('input', null, { 'class':'firepad-dialog-input', 'id':id, 'type':'text', 'placeholder':placeholder, 'style': 'display: none' });
   var fileinput = utils.elt('input', null, { 'class':'firepad-file-input', 'id':'file-input', 'type':'file', 'style': 'display: none', 'data-object_type':'UserMedia' });

   var submit = utils.elt('a', 'Insert', { 'class': 'firepad-btn', 'id':'submitbtn' });
   var selectFile = utils.elt('a', 'Choose Image', { 'class': 'firepad-btn', 'id':'choose-btn' });

   utils.on(submit, 'click', utils.stopEventAnd(cb));

   var cancel = utils.elt('a', 'Cancel', { 'class': 'firepad-btn firepad-cancel' });

   utils.on(cancel, 'click', utils.stopEventAnd(hideDialog));

   utils.on(selectFile, 'click', utils.stopEventAnd(performClick));

   utils.on(fileinput, 'change', utils.stopEventAnd(filename));

  var imgPreview = utils.elt('img', null, { 'class':'firepad-img-preview', 'id':'img-preview', 'src':'/img/no_image_available.jpg', 'style': 'width: 100px; height: 100px;'});

  var buttonsdiv = utils.elt('div', [selectFile, fileinput, submit, cancel], { 'class':'firepad-btn-group' });

  var previewdiv = utils.elt('div', [imgPreview], { 'class':'firepad-btn-group pull-left'});

  var cleardiv = utils.elt('div', null, { 'class':'firepad-btn-group' });

  var errordiv = utils.elt('div', null, { 'id':'notification-img-upload' });


  var div = utils.elt('div', [input, previewdiv, cleardiv, buttonsdiv, errordiv], { 'class':'firepad-dialog-div' });
