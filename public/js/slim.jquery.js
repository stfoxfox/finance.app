...
/**
 * Returns the mimetype matching the geiven extension
		...
		value: function _init() {
			...
			if (this._isImageOnly()) {
				...
				if (!this._input.hasAttribute('accept') || this._input.getAttribute('accept') === 'image/*') {
					...
					mimetypes = this._input.accept.split(',').filter(function (mimetype) {
...
(function () {
	...
	Slim.getOptionsFromAttributes = function (element) {
		...
		for (var prop in defaults) {
			...
			var value = defaults[prop](element, getData(element, toAttribute(prop)));
	...
	function endpointToMethod(endpoint) {
		return scope + (endpoint === 'create' ? '' : capitalizeFirstLetter(endpoint));
	}
	...
	function addTojQuery(endpoint) {
		$.fn[endpointToMethod(endpoint)] = function() {
			var args = argsToArray(arguments);
			var results = [];
			this.each(function(){
				results.push({
					element:this,
					result:Slim[endpoint].apply(Slim[endpoint],[this].concat(args))
				});
			});
			return results;
		}
