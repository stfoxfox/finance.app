...
tinymce.PluginManager.add('media', function(editor, url) {
	...
	function showDialog() {
		...
		var generalFormItems = [
			{
				...
				spacing: 3,
				items: [
					{type: 'container', html: '<button id="pasteFromCloud"><img src="/img/cloud/upload.png"></button>',onclick: openCloudIframe},
					{name: 'source1', id: 'inpCloudSource', type: 'filepicker', filetype: 'media', size: 40, autofocus: true, onchange: function(e) {
		...
		function openCloudIframe(e){
			// Open window
			editor.windowManager.open({
				title: 'Мои видео файлы',
				width: 600,
				height: 400,
				file : '/Cloud/fortinyVideo',
				buttons: [
					{
						text: 'Insert',
						onclick: function() {
							// Top most window object
							var win = editor.windowManager.getWindows()[1];
							var winParent = editor.windowManager.getWindows()[0];
							...
							if(winElement){
								...
								if(winElementType=='file'){
									var link = winElementUrl;
									$(winParent.$el).find('#inpCloudSource input').val(link);
									win.close();
								}else{
