			...
			<div class="block-normal bot-rounded-corners lightShadow bot-margin">
				<div class="block-content-main">
					...
					<ol class="breadcrumb">
					  ...
					  <li class="active"><?php echo lang("short_bc_edit_ticket_cat") ?></li>
					</ol>

					<p><?php echo lang("pg_sp_edit_ticket_cat_text") ?></p>
					...
					<table width="100%">
						...
						<?php
							foreach($categories->result() as $r) {
								echo"<tr class='table-row'><td>{$r->name}</td><td><a href='".site_url("sp_panel/delete_ticket_category/" . $r->ID . "/" . $this->security->get_csrf_hash())."' onclick=\"return confirm('".lang("pg_sp_delete_confirm_cat")."')\">".lang("pg_sp_link_delete")."</a> - <a href='".site_url("sp_panel/edit_ticket_category_pro/" . $r->ID . "/") ."'>".lang("pg_sp_link_edit")."</a></td></tr>";
							}
						?>
					</table>
				</div>
