				...
				<div class="block-content-main">
					...
					<form class="form-inline" role="form">
					  <div class="form-group">
					    ...
					    <input type="text" class="form-control" name="search_term" id="exampleInputEmail2" placeholder="<?php echo lang("pg_sp_edit_user_search_ph") ?>">
					  </div>
					  <div class="form-group"> 
					    <select name="type" class="form-control"><option value="0"><?php echo lang("pg_sp_name") ?></option><option value="1"><?php echo lang("pg_sp_email") ?></option><option value="2"><?php echo lang("pg_sp_ip") ?></option></select>
					  </div>
					...
					<table width="100%">
						...
						<?php
							foreach($users->result() as $r) {
								echo"<tr class='table-row'><td>{$r->name}</td><td>{$r->email}</td><td>{$r->IP}<br />". $this->common->getAccessLevel($r->access_level)."</td><td><a href='".site_url("sp_panel/delete_user/" . $r->ID . "/" . $this->security->get_csrf_hash())."' onclick=\"return confirm('".lang("pg_sp_delete_user_confirm")."')\">".lang("pg_sp_link_delete")."</a> - <a href='".site_url("sp_panel/ban_user/" . $r->ID . "/" . $this->security->get_csrf_hash())."' onclick=\"return confirm('".lang("pg_sp_ban_confirm")."')\">".lang("pg_sp_link_ban")."</a> - <a href='".site_url("sp_panel/edit_user_pro/" . $r->ID . "/") ."'>".lang("pg_sp_link_edit")."</a></td></tr>";
							}
						?>
					</table>
					<?php echo $this->pagination->create_links(); ?>
